...
Adresat PlikZAdresatami::pobierzDaneAdresata(string daneAdresataOddzielonePionowymiKreskami)
{
    ...
    for (int pozycjaZnaku = 0; pozycjaZnaku < daneAdresataOddzielonePionowymiKreskami.length(); pozycjaZnaku++)
    {
        ...
        else
        {
            ...
            {
            case 1:
                pobraneId = atoi(pojedynczaDanaAdresata.c_str());
                adresat.ustawId(pobraneId);
                break;
            case 2:
                pobraneId = atoi(pojedynczaDanaAdresata.c_str());
                adresat.ustawIdUzytkownika(pobraneId);
                break;
            case 3:
...
void PlikZAdresatami::usunWybranegoAdresataWPliku(int idUsuwanegoAdresata)
{
    ...
    if (odczytywanyPlikTekstowy.good() == true && idUsuwanegoAdresata != 0)
    {
        ...
        {
            idOdczytanegoAdresata = pobierzIdAdresataZDanychOddzielonychPionowymiKreskami(wczytanaLinia);
...
void PlikZAdresatami::dopisz(string tekst){
    ...
    if(plikTekstowy.good()){
        if(czyPlikJestPusty()){
            cout << "To jest poczatek pliku" << endl;
        }
